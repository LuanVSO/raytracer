...
int main()
{
	...
	constexpr vec3 horizontal{ view_width,0,0 };
	constexpr vec3 vertical{ 0,view_height,0 };
	constexpr vec3 lower_l_corner{ origin - horizontal / 2 - vertical / 2 - vec3(0,0,focal_length) };
	...
	for (int j = img_height - 1; j >= 0; j--)
	{
		...
		for (int i = 0; i < img_width; i++)
		{
			...
			const ray r(origin, lower_l_corner + u * horizontal + v * vertical - origin);
